...
func (i *Injury) RemoveInjuries(pawnID string) {
	...
	printer.Debugf("All injuries has been removed for {{{-BOLD,GREEN}}}%s", pawnID)
}
...
func (i *Injury) Remove(pawnID string, injuryIds []string) {
	...
	for i := 0; i < totalInjuries; i++ {
		for _, j := range injuryIds {
			...
			printer.Debugf("Injury {{{-RED,BOLD}}}%s {{{-RESET}}}removed", currentInjury.LoadId)
...
func (i *Injury) List(pawnID string) {
	...
	printer.Debugf("{{{-BOLD,MAGENTA}}}%s{{{-RESET}}}'s injuries:", pawnID)
	...
	for i := iterator.NewSliceIterator[*generated.Hediffs](p.HealthTracker.HediffSet.Hediffs); i.HasNext(); i = i.Next() {
		v := i.Value()
		printer.Debugf("%v ({{{-BOLD}}}load ID: %v{{{-RESET}}}) with a severity of {{{-RED}}}%f", v.Def, v.LoadId, v.Severity*100.0)
