...
type CustomType struct {
	...
	// It can be anything from reflect.Kind, *CustomType, *domain.Element, etc.
	Type1      any
	Type2      any
	ImportPath string
}
...
func createEmptyType() any {
	return &CustomType{
		...
		Pkg:        "*primary",
		Type1:      nil,
		ImportPath: importpath.PrimaryTypesPath,
	}
}
...
func createCustomSlice(logger logging.Logger, e *domain.Element, flag uint) any {
	return &CustomType{
		Name:       "Slice",
		Pkg:        "*types",
...
func createMultipleType() any {
	return &CustomType{
		...
		Pkg:        "*multiple",
		Type1:      nil,
		ImportPath: importpath.MultipleTypesPath,
	}
}
...
// If the type is invalid, we consider it as a *domain.Element.
// Is it useful for empty tags.
func determineTypeFromData(logger logging.Logger, e *domain.Element, flag uint) any {
	...
	if t == reflect.Struct || t == reflect.Slice {
		...
		if support.IsListTag(c.Child.GetName()) {
...
func createCustomTypeForMap(logger logging.Logger, e *domain.Element, flag uint) any {
	if e.Child == nil {
...
func findFirstValidElementWithChild(e *domain.Element) *domain.Element {
	n := e
...
func determineArrayOrSliceKind(_ logging.Logger, e *domain.Element) reflect.Kind {
	k := e
	for k != nil {
...
func getTypeFromArrayOrSlice(logger logging.Logger, e *domain.Element) reflect.Kind {
	...
	if k.Next != nil && k.Next.GetName() == k.GetName() {
		...
		for n != nil && n.GetName() == k.GetName() {
			if n.Child != nil {
				if support.IsListTag(n.Child.GetName()) {
	...
	if k.Child != nil {
		...
		if support.IsListTag(k.Child.GetName()) {
	...
	if kt == reflect.Invalid && e.Next != nil && support.IsListTag(e.Next.GetName()) && e.Next.GetName() == e.GetName() {
